...
def visualize_images(img_in, t_cost, params_cam, angle, img_name):
    '''
...
def img_line_central_point(left_fitx,left_fity,right_fitx,right_fity):
    central_x = (left_fitx[left_fitx.size/2]+right_fitx[right_fitx.size/2])/2
    central_y = (left_fity[left_fity.size/2]+right_fity[right_fity.size/2])/2
    return central_x, central_y

def draw_central_point_line_img(img,central_x,central_y,base_x,base_y):
    point_np = np.copy(img)
    point_np = cv2.circle(point_np,(central_x,central_y),5,(0,255,0),5)
    point_np = cv2.line(point_np,(base_x,base_y),(central_x,central_y),(0,255,255),5)
    return point_np
def calculate_angle(central_x,central_y,base_x,base_y):
    x = central_x - base_x
    y = -(central_y - base_y)
